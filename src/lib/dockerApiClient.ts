 ...
 * Calls local Docker API server using centralized configuration
 */
...
export async function fetchMarketData(options: FetchMarketDataOptions) {
  try {
    logApiCall(ENDPOINTS.MARKET_DATA, 'POST', options);
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: createApiError(errorData.error || `HTTP ${response.status}`, ENDPOINTS.MARKET_DATA, response.status)
      };
    }
    ...
    return {
      data: null,
      error: createApiError(error.message || 'Failed to connect to Docker API', ENDPOINTS.MARKET_DATA)
    };
  }
}
...
export async function testApiConnection(options: TestApiConnectionOptions) {
  try {
    logApiCall(ENDPOINTS.TEST_CONNECTION, 'POST', options);
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: createApiError(errorData.error || `HTTP ${response.status}`, ENDPOINTS.TEST_CONNECTION, response.status)
      };
    }
    ...
    return {
      data: null,
      error: createApiError(error.message || 'Failed to connect to Docker API', ENDPOINTS.TEST_CONNECTION)
    };
  }
}
...
export async function runBacktest(options: BacktestOptions) {
  try {
    logApiCall(ENDPOINTS.BACKTEST, 'POST', options);
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: createApiError(errorData.error || `HTTP ${response.status}`, ENDPOINTS.BACKTEST, response.status)
      };
    }
    ...
    console.error('[DOCKER API] Backtest error:', error);
    return {
      data: null,
      error: createApiError(error.message || 'Failed to run backtest', ENDPOINTS.BACKTEST)
    };
  }
}
...
export async function optimizeStrategy(options: OptimizeOptions) {
  try {
    logApiCall(ENDPOINTS.OPTIMIZE, 'POST', options);
    ...
    if (!response.ok) {
      ...
      return {
        data: null,
        error: createApiError(errorData.error || `HTTP ${response.status}`, ENDPOINTS.OPTIMIZE, response.status)
      };
    }
    ...
    console.error('[DOCKER API] Optimization error:', error);
    return {
      data: null,
      error: createApiError(error.message || 'Failed to optimize strategy', ENDPOINTS.OPTIMIZE)
    };
  }
}
    ...
    invoke: async (functionName: string, options: any) => {
      switch (functionName) {
        ...
        default:
          ...
          return {
            data: null,
            error: createApiError(`Function ${functionName} not implemented`, 'unknown')
          };
      }
    }
  },
